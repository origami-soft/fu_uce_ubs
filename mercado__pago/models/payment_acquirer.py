...
class payment_acquirer_mercadopago(models.Model):
    ...
    mp_service_mode = fields.Selection([
                                            ("basic","Básico"),
                                            ("custom","Personalizado")
                                        ], 
                                        string='Modo Checkout', 
    ...
    for_multicompany = fields.Boolean(string='Multicompañia')
    ...
    def render(self, reference, amount, currency_id, partner_id=False, values=None):
        ...
        if(_payment_transaction):
            if(_payment_transaction.acquirer_id.provider == "mercadopago"):
                ...
                try:
                    _payment_transaction.sudo().update({'last_collector_id': None, 'mp_json_response': None, 'mp_response': None})
                except:
                    pass
        ...
        try: 
            ...
            if(websites):
                for website in websites:
                    if(order_id):
                        _order = self.env['sale.order'].sudo().search([('id','=',int(order_id))], limit=1)
                    ...
                    _logger.warning('DRAFT ORDERS')
                    _logger.warning(acquirer.id)
                    _logger.warning(acquirer.name)
                    _logger.warning(acquirer.website_id.name)
                    ...
                    if(order_id):
                        ...
                        pending_sale_orders = self.env['sale.order'].sudo().search([
                                                                                    ['last_collector_id','!=',None],
                                                                                    ['website_id','=',int(website.id)]
                                                                                   ], 
                                                                                   order='id asc')
                    
                    _logger.warning('WEBSITE')
                    _logger.warning(website.id)
                    ...
                    for sale_order in pending_sale_orders:
                        if(sale_order.state == 'draft' or sale_order.state == 'sent'):                   
                            if(sale_order.last_collector_id):
                                ...
                                _logger.warning('PAYMENT FILTER')
                                _logger.warning({'external_reference':str(sale_order.id), 'sort':'date_last_updated','criteria':'desc'})
                                payment_info = mp.search_payment(filters={'external_reference':str(sale_order.id), 'sort':'date_last_updated','criteria':'desc'}, offset=str(0), limit=str(1))

                                _logger.warning('mercadopago - search_payment')
                                _logger.warning(payment_info)

                                collection_id = None

                                if(payment_info['status']==200 or payment_info['status']==201):
                                    if(len(payment_info['response']['results'])>0):
                                        collection_id = payment_info['response']['results'][0]['collection']['id']
                                    else:
                                        collection_id = None
                                ...
                                if(collection_id):
                                    ...
                                    if(payment_info['status']==200 or payment_info['status']==201):
                                        ...
                                        # payment_info['status_detail'] = "accredited"

                                        _logger.warning("MERCADOPAGO - query")
                                        ...
                                        _logger.warning("sale_order_transaction_rel")
                                        _logger.warning(sale_order_transaction_rel)
