...
#./odoo-bin -u mercadopago -c /etc/odoov13-server.conf -d odoov13 --stop-after-init --dev all --dev reload

class mercadopago_acquirer(http.Controller):
    ...
    def get_mercadopago_acquirer(self, **kw):  
        _logger.warning("request.website")
        _logger.warning(request.website)
        response = {"acquirer":None,"form_bill":None}
        query = "select name, website_id,company_id, state, provider, mp_service_mode, mp_client_id, mp_public_key, mp_country from payment_acquirer where provider = 'mercadopago' and website_id = "+str(request.website.id)+" limit 1"
        request.cr.execute(query)
        acquirer = request.cr.dictfetchone()
        response = { 
                        "acquirer":acquirer,
                        "bill_form": self.file_get_contents(str(self.formsPath)+str("/static/src/form/bill.html")), 
                    }
        return response    
    ...
    def get_sale_order(self, **kw): 
        ...
        query = "select name, website_id,company_id, state, provider, mp_service_mode, mp_client_id, mp_client_secret, mp_public_key, mp_access_token, mp_country from payment_acquirer where provider = 'mercadopago'  and website_id = "+str(request.website.id)+"  limit 1"
        ...
        query = "select id, name, amount_total, amount_tax, date_order, partner_shipping_id from sale_order where partner_id = '"+str(params['partner_id'])+"' and website_id = "+str(request.website.id)+" and state = '"+str('draft')+"' order by date_order desc limit 1"
        ...
        if(draft_order):                                   
            ...
            if(acquirer['mp_service_mode']=="basic"):
                ...
                _logger.warning("preference response")
            ...
            _logger.warning("get_sale_order response")
        ...
        try:
            ...
            if(_response['collection_status']==str("pending") 
                ...
                if(int(_response['external_reference'])>0):
                    ...
                    query = "select id from payment_acquirer where provider = 'mercadopago'  and website_id = "+str(request.website.id)+"  limit 1"
                    request.cr.execute(query)
                    acquirer = request.cr.dictfetchone()
